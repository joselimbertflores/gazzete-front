...
export class UserImporter {
  ...
  readonly form = new FormGroup({
    candidate: new FormControl<IdentityCandidateResponse | null>(null, Validators.required),
    roles: new FormControl<UserRole[]>([UserRole.USER], {
      validators: Validators.required,
    }),
  });

  private readonly selectedCandidateSignal = toSignal(this.form.controls.candidate.valueChanges, {
    initialValue: this.form.controls.candidate.value,
  });

  readonly selectedCandidate = computed(() => this.selectedCandidateSignal());
  ...
  save(): void {
    ...
    const { candidate, roles } = this.form.value;

    if (!candidate || !roles) return;

    this.isSaving.set(true);
    this.importError.set(null);

    this.userApi
      .importFromIdentity(candidate.externalKey, roles)
